...
**DistZKBench** 是一个双平台 distributed PCS/SNARK benchmarking framework：

```text
Tier-1: Linux strict artifact backend
  cgroup v2 + cpuset + resctrl CAT + netns/tc + perf_event_open
  用于论文主实验和严格 artifact evaluation。

Tier-2: macOS Apple Silicon backend
  process isolation + TCP transport + QoS control + Mach/libc measurement
  + user-space network shaper
  用于开发、可移植性测试、补充实验、轻量复现实验。
```

macOS backend 不是 Linux strict backend 的等价替代。它保留 protocol-level measurement
语义，但 CPU、memory、cache、network emulation、perf counters 等资源控制必须报告为
best-effort 或 unsupported。
...
4. platform-specific resource isolation and capability reporting
...
严格 artifact backend 支持 Linux，并显式依赖 Linux 特性：
...
同时提供 Apple Silicon / M-series macOS backend，定位为 portability /
developer benchmarking / secondary artifact path。macOS backend 必须明确标注：

```text
process-per-rank execution: supported
TCP-only data plane: supported
star/full-mesh topology enforcement: supported
proof size and verifier measurement: supported
phase tracing and communication accounting: supported
hard CPU affinity: unsupported
cgroup-style hard memory limit: unsupported
resctrl CAT / LLC partitioning: unsupported
network namespace / tc netem: unsupported
Linux perf_event counters: unsupported
```

Windows 不作为 MVP-3 支持目标。
...
每次 preflight 和 run report 都必须输出 capability matrix。框架禁止把 macOS
best-effort 语义伪装成 Linux strict isolation。

| Capability | Linux strict backend | macOS Apple Silicon backend |
| --- | --- | --- |
| OS process per prover rank | 支持 | 支持 |
| TCP-only data plane | 支持 | 支持 |
| star / full-mesh topology | 支持 | 支持 |
| CPU core pinning | `sched_setaffinity` / cpuset，严格 | 不严格；QoS + no-overcommit + workload policy |
| fixed worker thread count | 支持 | 支持 |
| memory hard limit | cgroup v2 `memory.max`，严格 | watchdog / `setrlimit` best-effort，非严格 RSS limit |
| memory measurement | cgroup + procfs + rusage | Mach `task_info` / rusage |
| cache isolation | resctrl CAT，硬件支持时严格 | 不支持严格 LLC isolation |
| network namespace | 支持 | 不支持 |
| tc netem | 支持 | 不支持；默认 user-space shaper，可选 pf/dnctl experimental |
| perf counters | `perf_event_open` | 不提供 Linux-equivalent perf；可选 powermetrics / Instruments / DTrace |
| NUMA binding | 支持 | 不适用；Apple Silicon unified memory 下不做 NUMA placement |
| remote SSH cluster | 支持 | 支持 |
| artifact headline results | 推荐 | 不推荐，除非明确标注 best-effort isolation |
...
## 5.3 Platform backend abstraction

平台相关能力必须通过统一 abstraction 暴露，不允许 controller 或 SDK 直接散落
`cfg(target_os)` 分支。

```rust
pub trait PlatformBackend {
    fn detect_capabilities(&self) -> Result<CapabilityReport>;

    fn prepare_host(&self, cfg: &ResolvedConfig) -> Result<HostPlan>;

    fn launch_rank(&self, spec: RankLaunchSpec) -> Result<RankHandle>;

    fn launch_verifier(&self, spec: VerifierLaunchSpec) -> Result<ProcessHandle>;

    fn setup_network(&self, plan: &TopologyPlan) -> Result<NetworkPlan>;

    fn start_measurement(&self, pids: &[Pid]) -> Result<MeasurementHandle>;

    fn sample_process(&self, pid: Pid) -> Result<ProcessSample>;

    fn stop_measurement(&self, handle: MeasurementHandle) -> Result<MeasurementReport>;

    fn cleanup(&self, run_id: RunId) -> Result<()>;
}
```
...
```text
target_os = linux  -> linux backend
target_os = macos  -> darwin backend
otherwise fail
```

`CapabilityReport` 必须包含：
...
Linux backend 的 `isolation_tier` 默认为 `strict`。Darwin backend 的
`isolation_tier` 必须为 `best_effort`。
...
## 6.3 macOS Apple Silicon 配置示例

本地 loopback TCP：

```yaml
experiment:
  name: toy_macos_loopback
  repetitions: 5
  warmups: 1
  random_seed: 12345
...
macos:
  qos_profile: user_initiated
  scheduling:
    max_ranks_policy: performance_cores_only
  thermal:
    require_nominal: true
    record_powermetrics: false

memory:
  macos_sampling: phase_boundary
  per_rank_limit: 8GiB
  macos_enforcement: watchdog
...
protocol:
  adapter: ./target/release/toy_star_adapter
  mode: sdk-binary
```

macOS user-space shaped network：

```yaml
network:
  transport: tcp
  mode: userspace_shaper
  base_port: 39000
  shaper:
    bandwidth: 10gbit
    latency: 50us
...
# 7A. macOS Apple Silicon backend

macOS backend 的定位是 portability / developer benchmarking / secondary
artifact path。它不得替代 Linux strict isolation path，也不得把 best-effort
resource controls 写成严格隔离。

## 7A.1 继续保留的严格语义

macOS backend 仍然必须严格保证：

```text
1. 每个 prover rank 是独立 OS process。
2. 每个 rank 有单独 TCP port。
3. star / full-mesh topology enforcement 由 framework transport 层执行。
4. 所有 protocol messages 走 measured TCP data plane。
5. proof size 是 canonical proof bytes。
6. verifier 独立 process 运行。
7. worker thread count / verifier thread count 由环境变量和 SDK 检查。
8. phase-level tracing 准确。
9. communication bytes 准确。
```
...
```text
1. hard CPU core pinning
2. hard memory cgroup limit
3. hard LLC cache isolation
4. Linux-like network namespace isolation
5. Linux tc netem equivalent
6. Linux perf_event equivalent
```
...
```json
{
  ...
  "isolation_tier": "best_effort",
  "cpu_isolation": "qos_best_effort_no_hard_affinity",
  "memory_enforcement": "watchdog_best_effort",
  "cache_isolation": "unsupported",
  "network_emulation": "userspace_shaper"
}
```

## 7A.3 CPU 调度策略

macOS 不使用 Linux 式 “rank i 绑定 core j” 语义。MVP-3 Darwin backend 使用：
...
macos:
  ...
  scheduling:
    no_overcommit: true
    max_ranks_policy: performance_cores_only
    # performance_cores_only | physical_cores | logical_cores
...
```json
{
  "cpu_control": {
    ...
    "no_overcommit": true,
    "rank_thread_budget": 1,
    "verifier_thread_budget": 1,
    "detected_performance_cores": 8,
    "detected_efficiency_cores": 4,
    "detection_method": "sysctl_best_effort"
  }
}
```
...
macOS 没有 cgroup v2 `memory.max` 等价物。Darwin backend 使用：

```text
Measurement:
  Mach task_info resident_size + getrusage。

Enforcement:
  watchdog best-effort；超过 budget 时 controller/agent kill rank。
```
...
memory:
  macos_sampling: phase_boundary
  # phase_boundary | interval_thread | external_best_effort
  per_rank_limit: 16GiB
  macos_enforcement: watchdog
  # watchdog | none | rlimit_as
```

`phase_boundary` 表示每个 phase_start / phase_end 调用一次 Mach task API 采样。
它不启动额外 sampler thread；如果配置 `interval_thread`，报告必须标记
`memory_sampling_overhead = rank_internal_sampler_thread`。
...
```json
{
  "memory": {
    ...
    "peak_resident_size_bytes": 1234567890,
    "limit_bytes": 17179869184,
    "limit_precision": "best_effort"
  }
}
```
...
macOS 不支持 strict LLC isolation。若用户在 macOS 请求：
...
```text
error: resctrl CAT is unavailable on macOS.
```

macOS 支持的 cache modes：

```yaml
cache:
  mode: none
  # none | cold_cache_best_effort
```

`cold_cache_best_effort` 可以在 measured run 前扫描大 buffer 尝试驱逐 cache，
但报告必须说明 cold-cache control is not cache isolation。

## 7A.6 Thermal / power monitoring

M-series macOS benchmark 必须记录 thermal variability：
...
macos:
  thermal:
    ...
    abort_on_state: serious
    record_powermetrics: true
    fail_on_thermal_contamination: false
```
...
## 7A.7 macOS performance counters

macOS 没有稳定的 Linux `perf_event_open` 等价 per-process counter。默认：
...
```yaml
metrics:
  collect_macos_powermetrics: true
```

则只作为 system-level supplemental metrics，并报告：

```json
"perf_counters": {
  "linux_perf_equivalent": false,
  "source": "powermetrics",
  "scope": "system_or_process_best_effort"
}
```
...
## 8.8 User-space network shaper

为了支持 macOS Apple Silicon 和无 root 开发环境，transport 层提供
user-space shaper。它不是 kernel-level packet shaping，不能替代 Linux
`netns_veth + tc` 的 strict artifact path，但适合 protocol-level sensitivity
测试和跨平台 toy / small experiments。
...
```text
1. 所有 protocol messages 仍走 TCP。
2. framework 在发送 frame 前进入 shaper。
3. 每条 logical edge 维护 token bucket。
4. latency/jitter 通过 delayed send queue 实现。
5. bandwidth 通过 paced frame writes 实现。
6. delayed time 计入 network_wait / send_time breakdown。
```
...
macOS 可选 `pf_dnctl` backend 只作为 experimental mode：
...
它需要 root，规则清理风险高，且不同 macOS 版本差异较大；不得作为默认
artifact headline path。

## 8.9 Remote SSH cluster
...
artifact/
  ...
  macos-apple-silicon/
    ...
    configs/
      macos_toy_star.yaml
      macos_toy_fullmesh.yaml
      macos_ours_small.yaml
      macos_userspace_shaper.yaml
    scripts/
      run_quick.sh
      check_results.py
```
...
cargo build --release --locked
dzb run artifact/main-linux/configs/toy.yaml
dzb report results/toy/
```

macOS Apple Silicon supplemental quickstart：

```bash
cargo build --release --locked
./target/release/dzb preflight artifact/macos-apple-silicon/configs/macos_toy_star.yaml
./target/release/dzb run artifact/macos-apple-silicon/configs/macos_toy_star.yaml
./target/release/dzb report results/toy_macos_star/
```

如果启用 powermetrics：

```bash
sudo ./target/release/dzb run artifact/macos-apple-silicon/configs/macos_powermetrics.yaml
```
...
macOS expected results 额外要求：

```text
report contains best-effort isolation warning
unsupported Linux-only features are marked unsupported, not failed
thermal state is recorded
QoS class is recorded
Mach resident memory peak is recorded
```
...
```text
We implement DistZKBench, a benchmarking framework for distributed PCS/SNARK protocols with two platform backends: a strict Linux backend and a best-effort macOS Apple Silicon backend. DistZKBench separates the benchmarking controller from protocol participants. The controller launches processes, assigns resources, constructs network topologies, synchronizes phases, and collects metrics, but it never participates in the cryptographic protocol and is excluded from all prover and verifier measurements.

On Linux, each prover rank is executed as an isolated OS process with fixed CPU affinity, cgroup-based memory limits, optional NUMA binding, and optional LLC isolation via Linux resctrl. Protocol messages are sent exclusively through a measured TCP data plane. DistZKBench supports both star and full-mesh topologies, which cover the dominant communication patterns in distributed ZKP systems. In star topology, rank 0 is the protocol master and may also act as a worker; all computation, memory usage, and communication performed by rank 0 are counted as prover cost.

The macOS Apple Silicon backend preserves the same protocol API, process-per-rank execution model, TCP-only transport, proof-size measurement, verifier measurement, phase tracing, and communication accounting. Because macOS does not expose Linux-equivalent cgroup memory limits, cpuset CPU pinning, resctrl LLC partitioning, network namespaces, or perf_event counters, macOS resource isolation is reported as best-effort and its results are labeled separately from Linux strict-isolation results.

The framework records prover time, verifier time, proof size, per-rank peak memory, per-edge communication, and phase-level compute/communication breakdowns. It also supports network namespace and tc-based single-machine emulation on Linux, user-space network shaping for portable experiments, and SSH-based small-cluster deployment for calibration.
```
...
[x] mio nonblocking TCP transport
[x] length-prefixed framed messages
[x] large-message chunking
[x] communication byte counters
[x] per-edge communication matrix
[x] phase tracing
...
# 32. macOS backend references

macOS Apple Silicon backend 的 QoS、Mach task memory、thermal state、powermetrics
和 optional pf/dnctl 规划参考：

```text
Apple Energy Efficiency Guide: Prioritize Work at the Task Level
...
FreeBSD dnctl manual
https://man.freebsd.org/cgi/man.cgi?format=html&query=dnctl&sektion=8
```
