...
ssh "${SSH_ARGS[@]}" "${DZB_LINUX_SSH}" "cd '${DZB_LINUX_WORKDIR}' && cargo build --release --locked && ./target/release/dzb preflight --config artifact/main-linux/configs/toy.yaml && ./target/release/dzb run artifact/main-linux/configs/toy.yaml"
