...
fn summarize_sweep(result_dir: &Path) -> Result<String, String> {
    ...
    let json = serde_json::to_string_pretty(&serde_json::json!({
        "schema_version": 2,
        "run_count": runs.len(),
        "ok_count": runs.iter().filter(|(_, run)| run.status == "ok").count(),
        "runs": values,
    }))
    ...
    let mut csv = String::from(
        "path,run_id,status,platform,isolation,proof_bytes,protocol_bytes,prover_ms,verifier_ms\n",
    );
    ...
    fs::write(
        ...
        format!(
            "<!doctype html><meta charset=utf-8><title>DistZKBench sweep</title><h1>DistZKBench sweep</h1><p>{} runs; {} ok</p><table><thead><tr><th>path</th><th>run</th><th>status</th><th>platform</th><th>isolation</th><th>proof bytes</th><th>protocol bytes</th><th>prover ms</th><th>verifier ms</th></tr></thead><tbody>{rows}</tbody></table>",
            runs.len(),
            runs.iter().filter(|(_, run)| run.status == "ok").count(),
        ),
    )
    .map_err(|error| error.to_string())?;
    Ok(format!(
...
fn collect_runs(dir: &Path, runs: &mut Vec<(std::path::PathBuf, RunJson)>) -> Result<(), String> {
    for entry in fs::read_dir(dir).map_err(|error| error.to_string())? {
        ...
        if path.is_dir() {
            ...
            runs.push((path.parent().unwrap_or(dir).to_path_buf(), run));
        }
    }
    Ok(())
}
