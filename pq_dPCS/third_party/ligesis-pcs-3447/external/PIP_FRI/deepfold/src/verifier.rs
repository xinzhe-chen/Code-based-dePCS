...
use crate::{prover::RandomOracle, VQueryResult};
...
impl<T: PrimeField> Verifier<T> {
    pub fn new(
        ...
        Verifier {
            ...
            shuffle_eval: None,
            deep_evals: vec![],
            open_point: (0..total_round).map(|_| T::rand(&mut thread_rng())).collect(),
            step,
        }
    }
    ...
    fn _verify(&self, polynomial_proof: &Vec<VQueryResult<T>>) -> bool {
                            ...
                            (x + nx
                                + challenge[j]
                                    ...
                                    * self.interpolate_cosets[i * self.step + j]
                                        .element(verify_inds[l]).inverse().unwrap())
                                * T::from(2 as u64).inverse().unwrap(),
                        );
